...
// This contract handles RPTR bridging between RaptorChain and Fantom (forked from polygon one, might be forked to other chains in the future)
// Thus, it allows trading RPTR on fantom-based DEXes (e.g. Spiritswap)
...
// - beacon block gets forwarded to Fantom-side handler
// - handler unpacks call and calls token contract
// - token contract mints token

// UNWRAP
// - user calls `unwrap` method
// - contract burns fantom-side token
// - contract writes data to a slot on fantom-side datafeed (slots can be accessed by raptorchain-side contracts)
